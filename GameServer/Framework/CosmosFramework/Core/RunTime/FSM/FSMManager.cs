...
namespace Cosmos.FSM
{
    ...
    /// </summary>
    public class FSMManager : Module,IFSMManager
    {
        #region Properties
        /// <summary>
        /// 单个状态机
        ...
        public void SetFSMSetRefreshInterval<T>(int interval)
            where T : class
        {
            Type type = typeof(T);
        ...
        /// <param name="interval">轮询间隔</param>
        public void SetFSMSetRefreshInterval(Type type, int interval)
        {
            if (HasFSMSet(type))
    ...
    where T : class
            ...
            where T : class
        {
            ...
            if (Individual)
            {
                ...
                fsmIndividualDict.TryAdd(type, fsm);
            }
            else
            {
                fsm = FSM<T>.Create(name, owner, states);
