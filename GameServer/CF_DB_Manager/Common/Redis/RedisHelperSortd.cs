...
namespace ProtocolCore
{
    public class RedisHelperSortd
    {
        ...
        /// <param name="score">排序分数，为空将获取集合中最大score加1</param>
        /// <returns></returns>
        public static async Task<long> SortedSetAddAsync(string key, string[] jsonVars, double? score = null)
        {
            try
            {
                var remote = MagicOnionClient.Create<IRedisHelperSorted>(GRPCManager.Instance.Channel);
                var result = await remote.SortedSetAddAsync(key, jsonVars, score);
                return await result;
            }
            catch (System.Exception e)
            {
