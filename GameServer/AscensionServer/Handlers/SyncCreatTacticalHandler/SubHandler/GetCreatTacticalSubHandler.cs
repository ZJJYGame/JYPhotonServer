   ...
   public class GetCreatTacticalSubHandler: SyncCreatTacticalSubHandler
    {
        ...
        public override OperationResponse EncodeMessage(OperationRequest operationRequest)
        {
            ...
            {
                //获取临时储存的阵法并移除
                var Exist = GameEntry.TacticalDeploymentManager.TryAddRemoveTactical(tacticObj.RoleID, out var tacticalDTO);
                if (Exist)
                {
                   GameEntry.TacticalDeploymentManager.GetRoleTactic(tacticObj.RoleID, out List<TacticalDTO> roletactical);
                    GameEntry.TacticalDeploymentManager.TryAddRoleAllTactical(tacticalDTO.RoleID, tacticalDTO);
                }
            }
            else
            {
